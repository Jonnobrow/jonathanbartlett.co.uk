#+HUGO_BASE_DIR: ~/local/personal_projects/blog.jonnobrow.co.uk
...
* TODO Ansible for... dotfiles?
:PROPERTIES:
:EXPORT_FILE_NAME: ansible-for-dotfiles
...
tl;dr
#+end_summary
I now manage my dotfiles with an Ansible playbook. It also handles updates for my OS and Doom Emacs.
This was inspired by [[https://www.jeffgeerling.com/][Jeff Geerling]], who authored Ansible for DevOps and Ansible for Kubernetes.
#+end_details

** Inspiration

During a boredom driven spiral through YouTube I stumbled across a [[https://www.youtube.com/watch?list=PL2_OBreMn7FqZkvMYt6ATmgC0KAGGJNAN&v=goclfp6a2IQ][series of videos]] by [[https://www.jeffgeerling.com/][Jeff Geerling]]
which go through basically everything you need to know about Ansible[fn:ansible] , especially for a hobbyist like me.
I had heard of Ansible[fn:ansible] prior to this point but had never really taken the time to learn about it,
and Jeff seemed like a reputable source so I started to watch.

During the series he mentions at least once about [[https://github.com/geerlingguy/mac-dev-playbook][managing his personal Mac with an Ansible playbook]]. So I set
about looking for ways to do that myself. This is when I found myself clicking through a repository by ~kespinola~ on
Github [fn:kespinola-dotfiles]. These seemed a little more my speed as they were targeted towards linux, so I used the
...
and I have used something like Chezmoi[fn:chezmoi] to manage my dotfiles, with some success. However Chezmoi[fn:chezmoi] is limited
when it comes to performing system operations, like installing packages, which I do on a regular basis when I install
a fresh copy of Arch Linux [fn:ArchLinux] on my machines. Therefore I have historically scripted this and had Chezmoi[fn:chezmoi]
run those scripts. The appeal of Ansible[fn:ansible] in this area is the idempotent nature of its operations, meaning
...
** Resources for Learning Ansible
** My Playbook
*** Roles
*** Script
*** Get my Playbook
...
[fn:chezmoi] https://www.chezmoi.io/
[fn:kespinola-dotfiles] https://github.com/kespinola/dotfiles
